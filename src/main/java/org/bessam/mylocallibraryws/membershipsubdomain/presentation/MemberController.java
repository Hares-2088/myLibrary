...
import org.bessam.mylocallibraryws.membershipsubdomain.business.MemberService;
...
public class MemberController {
    ...
    public ResponseEntity<List<MemberResponseModel>> getMembers() {
        return ResponseEntity.status(HttpStatus.FOUND).body(memberService.getMembers());
    }
    ...
    public ResponseEntity<MemberResponseModel> getMemberByMemberId(@PathVariable String memberId) {
        return ResponseEntity.status(HttpStatus.FOUND).body(memberService.getMember(memberId));
    }
